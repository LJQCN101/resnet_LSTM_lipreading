...
class I3D_BLSTM_mini(nn.Module):
    def __init__(self, inputDim=480, hiddenDim=256, num_lstm=2, nClasses=500):
        ...
        self.avg_pool = torch.nn.AvgPool3d((1, 14, 14), (1, 1, 1))
    ...
    def forward(self, x):
        ...
        out = self.avg_pool(out)                # batch_size x 480 x 14 x 1 x 1
        # print(out.size())
        # zxc

        out = out.squeeze(3)                    # batch_size x 480 x 14 x 1
        out = out.squeeze(3)                    # batch_size x 480 x 14

        out = out.transpose(1, 2)               # batch_size x 14 x 480
